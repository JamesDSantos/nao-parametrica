...
$$R=\{n_+: n_+\leq r\hbox{ ou } n_+\geq s \},$$
onde $s=n-r+1$ e $P(N_+\leq r)\leq \alpha/2$. O complementar de $R$ é $R^c=(r<n_+<s)$ e, pelo Teorema da dualidade entre testes de hipóteses e intervalos, temos que 
$$\begin{align}A(\theta)&=\{\theta:r<N_+<s\}=\left\{\theta:r<\sum_{i=1}^n I_{(\theta,\infty)}(X_i)<s\right\}\\&=\left\{\theta:r<\sum_{i=1}^n I_{(-\infty,X_i)}(\theta)<s\right\}\end{align}$$
Denotando por $X_{(1)}<\cdots<X_{(n)}$ as estatísticas de ordem, temos que:

* o maior valor de $\theta$ tal que $r<\sum_{i=1}^n I_{(-\infty,X_i)}(\theta)$ é $X_{(r)}$

* o menor valor de $\theta$ tal que $s>\sum_{i=1}^n I_{(-\infty,X_i)}(\theta)$ é $X_{(s)}$

Portanto, $(X_{(r)},X_{(s)})$ é um intervalo de confiança $1-\alpha$ para a mediana $\theta$.
...
Para $\alpha/2=0,025$, e considerando que $N_+\sim\hbox{Binomial}(20,1/2)$, temos que $P(N_+\leq 6)=0,025$, logo $r=6$ e $s=15$. As estatística de ordem $X_{(5)}=0,05$ e $X_{(15)}=0,16$. Portanto, um intervalo de 95% confiança para a mediana da radiação deste lote é (0,05 , 0,16). 
:::
