...
$$\begin{align*}
&H_0: P(X=0,Y=1)=P(X=1,Y=0) \\ &\Rightarrow    H_0: P(X=0,Y=1)+P(X=0,Y=0)=P(X=1,Y=0)+P(X=0,Y=0)\\
&\Rightarrow H_0:P(X=0)=P(Y=0)
\end{align*}$$
e, portanto, o primeiro conjunto de hipóteses implica em marginais iguais. 
...
Identificando $p_+=P(X=0,Y=1)$ e $p_-=1-p_+$, a hipótese nula do teste de McNemar é equivalente a 
    $$H_0:p_+=p_-$$
...
$$N_+\sim\hbox{Binomial}(b+c,1/2).$$
Contudo, é usual utilizar o TCL para realizar esse teste quando $b>20$. Nesses casos, teremos que, sob $H_0$

$$\frac{2N_+-(b+c)}{\sqrt{n'}}=\frac{(b-c)^2fd}{\sqrt{b+c}}\stackrel{D}{\to}N(0,1)$$
e
$$T=\frac{b-c}{b+c}\stackrel{D}{\to}\chi^2_1.$$
